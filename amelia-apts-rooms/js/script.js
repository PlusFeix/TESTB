...
function populateSwiper(items, swiperContainerClass, type) {
    ...
    items.forEach(item => {
        ...
        card.querySelector('.btn-details').addEventListener('touchend', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('Pulsante "Scopri di più" toccato (touch) per:', item.name);
            try {
                openDetailModal(item);
            } catch (error) {
                console.error('Errore nel touch del pulsante Scopri di più:', error);
                alert('Si è verificato un errore. Ricarica la pagina e riprova.');
            }
        });
        ...
        // Add touch event for image on mobile
        card.querySelector('.card-image-clickable').addEventListener('touchend', (e) => {
            e.preventDefault();
            e.stopPropagation();
            console.log('Immagine toccata (touch) per:', item.name);
            try {
                openDetailModal(item);
            } catch (error) {
                console.error('Errore nel touch dell\'immagine:', error);
                alert('Si è verificato un errore. Ricarica la pagina e riprova.');
            }
        });
...
function openDetailModal(item) {
    ...
    // Mostra il modal
    console.log('Impostazione display del modal...');
    modal.style.display = 'block';
    console.log('Modal display impostato su block, computed style:', window.getComputedStyle(modal).display);
    
    // Force the modal to be on top for mobile devices
    modal.style.zIndex = '10000';
    console.log('Z-index del modal impostato a 10000');
    
    // Aggiungi un piccolo delay per permettere al display di essere applicato prima della transizione
    setTimeout(() => {
        console.log('Aggiunta classe show al modal...');
        modal.classList.add('show');
        console.log('Classe show aggiunta, classList:', modal.classList.toString());
    ...
    console.log('Body scroll bloccato per mobile');
    
    // Add a small delay to ensure modal is visible on mobile devices
    setTimeout(() => {
        const modalContent = modal.querySelector('.modal-content');
        if (modalContent) {
            console.log('Scroll al modal content...');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (modalCloseButton) {
        ...
        // Add touch event for mobile compatibility
        modalCloseButton.addEventListener('touchend', (e) => {
            e.preventDefault();
            closeDetailModal();
        });
    }
...
// Add a function to debug modal visibility issues
function debugModalVisibility() {
    ...
    if (modal) {
        ...
        console.log('Element at modal center:', elementAtCenter);
